...
{
    //console.log(erp, haat, channel, field, distance, fs_or_dist, curve, flag);
...
!**********************************************************************/
    ...
    if (fs_or_dist == 1 || fs_or_dist == 3) 
    {
        ...
        else
        {
            ...
            if(haat < 30.)   { h[0] = 30.;   flag[7] = 1; }  // All HAAT below 30 meters are set to 30
            if(haat > 1600.) { h[0] = 1600.; flag[8] = 1; }  // All HAAT above 1600 are set to 1600
...
!***********************************************************************/
    ...
    else if (fs_or_dist == 2)
    {
        ...
        for (i = 0; i <= n_points; i++)   { h[i] = haat; f5050[i] = f5010[i] = f[i] = d[i] = 0.0; 
                                            ...
                                            // Next two lines added 5/13/20
                                            if(h[i] < 30.)        { h[i] = 30.;   flag[7] = 1; } // All HAAT below 30   are set to 30
                                            else if(h[i] > 1600.) { h[i] = 1600.; flag[8] = 1; } // All HAAT above 1600 are set to 1600
                                          }
        ...
        {
            if((field > f[i]&&(curve==0||curve==1)) || (field > f5010[i] && curve==2))  
            ...
            {      // Use the free space equation to find the field strength and distance
                ...
                if(distance < 0.001) { distance = 0.001; }  // 1 meter distance minimum avoids zero di
...
var pre = "<pre style='font-family: Candara; font-size:13pt; color:green;'>      ";

// function AddComment()
// {
//     var innermyspan = document.getElementById("DataHere").innerHTML;
//     document.getElementById("DataHere").innerHTML=innermyspan+comment;
// }  
...
xerp      = (myArgs[0] == '-') ? 1.0 : parseFloat(myArgs[0]);   //formERP.value; 
xhaat     = parseInt(myArgs[1]);   // formHAAT.value;
xchannel  = myArgs[2];   // formCHANNEL.value; 250, 9, 20
xdistance = (myArgs[3] == '-') ? 0.0 : parseInt(myArgs[3]);   // formDISTANCE.value;  
xfield    = (myArgs[4] == '-') ? 0.0 : parseInt(myArgs[4]);   // formFIELD.value;
xcurve    = myArgs[5];   // formCURVE.value; 0, 1, 2
//xfs_or_dist = parseInt(myArgs[6]); // formFSorDIST.value;
xflag = []; 


if(xfield > 0 && xdistance > 0) {
    xfs_or_dist = 3;
} else if(xfield > 0 && xdistance == 0) {
    xfs_or_dist = 2;
} else {
    xfs_or_dist = 1;
}

// if     (xfs_or_dist == 1) { xfield = 0.0;    }  // 1 = finding field
// else if(xfs_or_dist == 2) { xdistance = 0.0; }  // 2 = finding distance
// else if(xfs_or_dist == 3) { xerp = 1.0;      }  // 3 = finding FM ERP


var resulttt = tvfmfs_metric( xerp, xhaat, xchannel, xfield, xdistance, xfs_or_dist, xcurve, xflag);

//var resultDiv = document.getElementById('DataHere');
// Errors
if(xflag[3] == 1)       // { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(3);  }
// else if(xflag[4] == 1)  { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(4);  }
// else if(xflag[5] == 1)  { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(5);  }
// else if(xflag[2] == 1)  { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(2);  }
// else if(xflag[15] == 1) { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(15); }
// else if(xflag[16] == 1) { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(16); }
// else if(xflag[17] == 1) { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(17); }

// //else if(isNaN(resulttt)) { DataHere.style.borderColor='#FF7F00'; resultDiv.innerHTML = tvfmfs_comment(11); }

// // Warnings & messages
// else if(xflag[12] == 1) { DataHere.style.borderColor='#23FF33'; resultDiv.innerHTML = tvfmfs_comment(12); }
// else if(xflag[13] == 1) { DataHere.style.borderColor='#23FF33'; resultDiv.innerHTML = tvfmfs_comment(13); }
// else if(xflag[14] == 1) { DataHere.style.borderColor='#23FF33'; resultDiv.innerHTML = tvfmfs_comment(14); }

// else
// {
//DataHere.style.borderColor='#23FF33'; 

//Round resulttt to 3 decimal places
//Three places to the right of the decimal is already more than real-world accuracy.

if(xfs_or_dist == 1 || xfs_or_dist == 2) 

    resulttt = resulttt.toFixed(3);
else if(xfs_or_dist == 3) resulttt = resulttt.toFixed(6);

if     (xfs_or_dist == 1)  {  var textput = '<html><body>' + pre + ' Calculated field strength = <b>' + resulttt + ' dBu</b></pre></body></html>';  }
else if(xfs_or_dist == 2)  {  var textput = '<html><body>' + pre + ' Calculated Distance = <b>' + resulttt + ' km</b></pre></body></html>';         } 

else if(xfs_or_dist == 3)  
{
    var m=0;  
    var roundedERP = round_power(resulttt, m);  

    // Next line added 3/13/2020 to limit display to 3 decimal places
    roundedERP = roundedERP.toFixed(3);

    var textput = '<html><body>' + pre + ' Calculated ERP (rounded per Section 73.212) = <b>' + roundedERP + ' kW</b><br>      (FM 60 dBu Service Contour only)</pre><p></body></html><p>' + pre + ' <font size=-1 >Unrounded ERP = <b>' + resulttt + ' kW</b></font> </pre></body></html>\n';  
}
